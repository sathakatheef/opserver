...
resource "aws_lb" "this_alb" {
  ...
  subnets                          = ["${lookup(aws_subnet.this-pub-sn.subnet_public_ids, var.alb_product_roles1)}","${lookup(aws_subnet.this-pub-sn.subnet_public_ids, var.nlb_product_roles2)}"]       #########Public Subnets
  idle_timeout                     = "60"
...
resource "aws_lb" "this_nlb" {
  ...
  subnets                          = ["${lookup(aws_subnet.this-pub-sn.subnet_public_ids, var.nlb_product_roles1)}","${lookup(aws_subnet.this-pub-sn.subnet_public_ids, var.nlb_product_roles2)}"]        ##########Public Subnets
  idle_timeout                     = "60" 
...
########AWS ROUTE 53 RECORD CNAME RECORD FOR APPLICATION LOADBALANCER#########
resource "aws_route53_record" "this" {
  provider = "aws.localr53zone"
  count    = "${var.r53_count}"       ####If this count is 0, this resource wont be computed.
  type     = "CNAME"
  ttl      = "300"
  records  = ["aws_lb.this_alb.dns_name"]
  zone_id  = "${aws_route53_zone.this.zone_id}"
  name     = "${var.environment}"
}

########AWS ROUTE 53 RECORD CNAME RECORD FOR NETWORK LOADBALANCER#########
resource "aws_route53_record" "this_nlb" {
 count    = "${var.logging_enabled ? 0 : var.nlb_count}"      ###This is set to 0 by default. Change alb_count to 1 if A
LB to be computed.
 provider = "aws.localr53zone"
 type     = "CNAME"
 ttl      = "300"
 ...
 zone_id  = "${aws_route53_zone.this.zone_id}"
 name     = "${var.env_number}-${var.load_balancer_name}-nlb"
}
